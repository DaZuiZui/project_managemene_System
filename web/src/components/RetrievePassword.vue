    ...
    <div>
        <div style="margin: auto;width: 600px" id="enrolldate" class="animated zoomIn" v-cloak>
         ...
         <div class="form-group">
                    <label for="email">Email used when registering</label>  <span style="color:#FF0000;font-size:14px">{{this.userdataerror.emailerror}}</span>
                    <input type="email" class="form-control" id="email" name="email" v-model="userdata.email">
                </div>
                <div class="form-group">
                    <label for="username">phoneNumber used when registering</label>    <span style="color:#FF0000;font-size:14px">{{userdataerror.phoneNumbererror}}</span>
                ...
                <button class="btn btn-lg btn-primary btn-block" type="submit"  id="sub"  v-bind:hidden="formbutton" style="width:600px;height: 50px;"> change Password</button> 
                  ...
                  //Update verification judged to be true
                   ...
                   if (this.userformif){
                      //Update form button is not submittable
                       ...
                       this.$axios.post("http://127.0.0.1:8081/user/p/retrievepassword",
                       this.userdata 
                       ).then(function (response) {
                            if(response.data == "0x0001"){ 
                                alert("Successfully modified, go to login!")
                               window.location.href="http://127.0.0.1:8080/pu/user/login";
                            }
                            else{
                                alert("Sorry, we cannot prove that you are the owner of this account");
                                vm.formbutton = false;
                            }
                       })
                   }else{
